...
namespace Smocktest
{
    class Assesmentsreview
    {
        ...
        public IWebDriver assesmentsreview(IWebDriver driver, string IID, string Env,string begindate,string enddate, bool Assessmentreview)
        {
            ...
            if (Env.ToUpper() != "EXPL" && Env.ToUpper() != "DEV3" && Env.ToUpper() != "DEV5" && Env.ToUpper() != "DEV2" && Env.ToUpper() != "DEV1")
            {
                ...
                driver.FindElement(By.Id("ContentPlaceHolder1_txtUsername")).SendKeys(Env.ToUpper() + ConfigurationManager.AppSettings.Get("LOCRWR"));
            }
            else
            {
            ...
            if(!IsElementPresent(driver, By.XPath("//*[@id=\"Assessment Detailed History\"]/a")))
            {
                driver.FindElement(By.Id("btnNext")).Click();
            }
            driver.FindElement(By.XPath("//*[@id=\"Assessment Detailed History\"]/a")).Click();
            driver.FindElement(By.Id("btnNext")).Click();
            driver.FindElement(By.Id("btnNext")).Click();
            if (IsElementPresent(driver, By.Id("Documents_0__Status")))
            {
            ...
            driver.FindElement(By.Id("btnNext")).Click();
            Thread.Sleep(3000);
            driver.FindElement(By.Id("ddlLevelofCare")).Click();
            {
                SelectDropDown(driver, By.Id("ddlLevelofCare"), "MET");
            }
            Thread.Sleep(1000);
            driver.FindElement(By.Id("ddlLevelofCare")).Click();
            driver.FindElement(By.Id("ddlReasonNotMet")).Click();
            {
                SelectDropDown(driver, By.Id("ddlReasonNotMet"), "AA");
            }
            Thread.Sleep(1000);
            driver.FindElement(By.Id("ddlReasonNotMet")).Click();
            driver.FindElement(By.Id("txtStartDate")).Click();
            driver.FindElement(By.Id("txtStartDate")).SendKeys(begindate);
            Thread.Sleep(1000);
            driver.FindElement(By.Id("txtEndDate")).Click();
            driver.FindElement(By.Id("txtEndDate")).SendKeys(enddate);
            Thread.Sleep(1000);
